...
export interface PublicRecordTableProps {
...
const ValueCell: React.FC<{ value?: string | string[] }> = ({ value }) => {
  ...
  if (isEmpty) {
    return (
      <div className="text-[12px] leading-[18px] text-[#6B7280] py-2">No Record Found</div>
    );
  }
  ...
  return <div className="font-medium text-xs leading-[150%] -tracking-[0.03em] text-[#292524] py-2">{content}</div>;
};

export const PublicRecordTable: React.FC<PublicRecordTableProps> = ({ rows }) => {
  return (
